...
def render_chart(symbol, levels=None, buy_signals=None, sell_signals=None):
    ...
    try:
        ...
        if df.empty:
            st.warning(f"⚠️ No chart data available for {symbol}")
            return
    ...
    if levels and isinstance(levels, dict):
        support = levels.get("stoploss")
        resistance = levels.get("target")
        entry = levels.get("entry")

    support_display = (
        f"₹{support:.2f}" if support and pd.notna(support) else "N/A"
    )

    resistance_display = (
        f"₹{resistance:.2f}" if resistance and pd.notna(resistance) else "N/A"
    )

    entry_display = (
        f"₹{entry:.2f}" if entry and pd.notna(entry) else "N/A"
    )

    # =========================
    # MARKERS (Safe handling)
    # =========================
    markers = []
    seen_dates = set()
    ...
    # =========================
    import hashlib
    chart_id = hashlib.md5(f"{symbol}_{time.time()}".encode()).hexdigest()[:8]

    # =========================
    # HTML (UPDATED WITH BETTER INITIALIZATION)
    # =========================
    chart_html = f"""
    <!DOCTYPE html>
    <html>
    ...
    <body>
        ...
        <div class="chart-container-wrapper">
            ...
            <div id="chart-container-{chart_id}"></div>
        ...
        <script>
            (function() {{
                const container = document.getElementById('chart-container-{chart_id}');
                ...
                if (typeof LightweightCharts === 'undefined') {{
                    container.innerHTML =
                        '<div style="color:white;padding:20px;">Chart library failed to load. Please refresh.</div>';
                    return;
                }}

                const fullCandles = {json.dumps(candles)};
                const fullVolumes = {json.dumps(volumes)};
                ...
                function initChart() {{
                    try {{
                        const chart = LightweightCharts.createChart(container, {{
                            ...
                            grid: {{
                                vertLines: {{ color: 'rgba(0,255,255,0.06)' }},
                                horzLines: {{ color: 'rgba(0,255,255,0.06)' }},
                            }},
                            crosshair: {{
    ...
    # Add price lines
    if support and pd.notna(support):
        chart_html += f"""
    ...
    if resistance and pd.notna(resistance):
        chart_html += f"""
                        candleSeries.createPriceLine({{
                            price: {resistance},
                            color: '#ff1744',
    ...
    chart_html += f"""
                        function filterData(data, timeframe) {{
                            ...
                            switch(timeframe) {{
                                case '1M': cutoff = new Date(lastDate.getTime() - 30 * DAY); break;
                                case '3M': cutoff = new Date(lastDate.getTime() - 90 * DAY); break;
                                case '6M': cutoff = new Date(lastDate.getTime() - 180 * DAY); break;
                                case '1Y': cutoff = new Date(lastDate.getTime() - 365 * DAY); break;
                                case '2Y': cutoff = new Date(lastDate.getTime() - 730 * DAY); break;
                                default: return data;
                            }}
                        ...
                        function setTimeframe(tf) {{
                            const candleData = filterData(fullCandles, tf);
                            const volumeData = filterData(fullVolumes, tf);
                        ...
                        container.innerHTML = '<div style="color:white;padding:20px;">Error loading chart. Please refresh.</div>';
                    }}
                }}
